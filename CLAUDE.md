...
read-out + fleet scan, full API debug runner + inverter-settings reader); **Hyper tab** (chart any raw inverter
parameter over the day at 5-min resolution from the expanded `dayexcel` CSV parser). **No open code tasks** — the
Dotsikas export issue is diagnosed as inverter-side and intentionally left alone in the app.
...
**Full CSV column layout** (confirmed from the header): `Time, MPPT1, MPPT2, MPPT3, PV, Temperature, E-Today, E-Total, H-Total, Grid1, Normal Load1, Gen Port1, AC OUT(100A)1, Smart LoadB(50A)1, Smart LoadC(30A)1, Grid2, Normal Load2, Gen Port2, AC OUT(100A)2, Smart LoadB(50A)2, Smart LoadC(30A)2, GridFac, LoadFac, GenFac, Feed-In Energy Today, Purchased Energy Today, Consumption Today, Total Feed-In Energy, Total Purchased Energy, Total Consumption, Capacity, BMS_Version, SOC, SOH, BatteryTemp, Battery Current, Battery Voltage, Battery Power, Daily charge energy, Daily discharge energy, Total charge energy, Total discharge energy, smartLoadDay, smartLoadTotal, Outputs Energy Today, Outputs Energy Total`. Cells are `"V/A/W"` (parse first field for volts, last for watts). `Grid1`/`Grid2` = the two grid legs' **line-to-neutral voltage** (split-phase 120/240 V → ~120 V each leg); `GridFac` = grid frequency. The proxy locates columns **by header name** (with positional fallbacks) via the `ix(name,fallback)` helper — never hard-code indices.

`dayexcel` returns, per 5-min row: the legacy `mppt[]`/`gridV[]`/`gridHz` (Day chart) **plus a flat metric map** (`pvW`, `mppt{1,2,3}{V,A,W}`, `gridL{1,2}{V,W}`, `loadL{1,2}W`, `genL{1,2}W`, `batV/batA/batW`, `gridHz/loadHz/genHz`, `soc/soh`, `temp/batTemp`) and a **`metrics` catalog** (`[{key,label,unit,group}]`) listing only the parameters that actually carry data that day. Parse helpers: `vOf` (first `/` field = volts), `aOf` (middle = amps), `wOf` (last = watts), `numOf` (scalar columns). This feeds the **Hyper** tab (below).

### Hyper tab — chart any parameter over the day (5-min resolution)
A dedicated **Hyper** tab (between Day and Month) renders `HyperChart`: pick one or more raw inverter parameters from grouped chips (Power / Voltage / Current / Frequency / Battery / Temperature) and plot them over the day at 5-min resolution. **Single inverter only** (the CSV is per-inverter — multi-select shows a "pick one" prompt). Up to **two distinct units** share a left + right Y-axis (selecting a third unit hides those traces with a note until a slot frees). Data comes straight from `dayexcel`'s flat metric map + `metrics` catalog (the catalog drives the chips, so empty params like generator legs don't appear). Date nav + zoom `<Brush>` like the Day chart; selection persists across day navigation, pruned/seeded against each day's available metrics. Colors from `HYPER_COLORS` (assigned by catalog index, stable across toggles).
