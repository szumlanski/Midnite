...
//
// Email is sent via the FIRST configured transport, in priority order:
//   1. SMTP   — any SMTP server (Gmail/Google Workspace app password, etc.).
//               No domain verification needed → the pre-launch "send now" path.
//   2. Resend — HTTP API (https://resend.com); needs a verified domain to email
//               arbitrary recipients (onboarding@resend.dev only reaches your own
//               Resend-account email).
// If neither is configured, send() NO-OPS SAFELY (returns {ok:false,skipped:true})
// — it never throws and never blocks the heartbeat/digest crons. The channel stays
// abstracted (send({channel,...})) so SMS/push can be added later.
//
// Env:
//   SMTP_HOST / SMTP_PORT / SMTP_USER / SMTP_PASS — SMTP relay (e.g. Gmail:
//     SMTP_HOST=smtp.gmail.com SMTP_PORT=465 SMTP_USER=you@yourdomain
//     SMTP_PASS=<16-char app password>). SMTP_SECURE defaults to true for :465.
//   RESEND_API_KEY     — Resend API key. Used only if SMTP isn't configured.
//   ALERTS_FROM_EMAIL  — From address, e.g. "Midnite Sentinel <alerts@you.com>".
//                        For SMTP, Gmail rewrites it to the authenticated user
//                        unless it's an allowed alias; defaults to SMTP_USER /
//                        Resend's onboarding sender.
...
export function channelConfigured(channel = "email") {
  if (channel === "email") return smtpConfigured() || !!process.env.RESEND_API_KEY;
  return false; // sms/push not wired yet
}

function fromAddress() {
  return process.env.ALERTS_FROM_EMAIL
    || (smtpConfigured() ? process.env.SMTP_USER : null)
    || "Midnite Sentinel <onboarding@resend.dev>";
}

// ── SMTP transport (nodemailer) — lazily created + cached so the module stays
// import-safe even where SMTP isn't configured. nodemailer is required lazily so
// it's never pulled into a client bundle. ──────────────────────────────────────
let _smtp = null;
...
async function sendViaSMTP({ to, subject, html, text }) {
  try {
    ...
    return { ok: true, skipped: false, providerId: info?.messageId || null };
  } catch (e) {
    return { ok: false, skipped: false, reason: "smtp: " + (e.message || String(e)) };
  }
}
...
async function sendEmail({ to, subject, html, text }) {
  ...
  return { ok: false, skipped: true, reason: "no email transport configured (set SMTP_* or RESEND_API_KEY)" };
}
...
// Site-share notification — recipient already has a Sentinel account.
export function buildShareMessage({ ownerName, siteName }) {
  const who = ownerName || "A Midnite Sentinel user";
  const title = `${who} shared a site with you`;
  const lines = [
    `<strong>${who}</strong> shared the site <strong>${siteName}</strong> with you.`,
    `It's available now in your ${BRAND} account — open the app and switch to it from the account selector (view-only).`,
  ];
  const text = `${who} shared the site "${siteName}" with you on ${BRAND}. Open ${APP_URL} to view it.`;
  return { subject: `${BRAND} — ${who} shared ${siteName} with you`, html: shell(title, lines), text };
}

// Site-share INVITE — recipient doesn't have an account yet; prompt them to sign up with this email.
export function buildShareInviteMessage({ ownerName, siteName, email }) {
  const who = ownerName || "A solar site owner";
  const title = `${who} invited you to view a solar site`;
  const lines = [
    `<strong>${who}</strong> wants to share their site <strong>${siteName}</strong> with you on ${BRAND}.`,
    `Create a free account using <strong>${email}</strong> and the site will appear automatically — view-only, no equipment access.`,
  ];
  const text = `${who} shared the solar site "${siteName}" with you on ${BRAND}. Sign up at ${APP_URL} using ${email} to view it.`;
  return { subject: `${BRAND} — you're invited to view ${siteName}`, html: shell(title, lines), text };
}
