...
export default async function handler(req, res) {
  ...
  try {
    ...
    switch (action) {
      ...
      case "shadow": {
        // IoT device-shadow poke. setShadowCommand writes a command into the unit's
        // Aliyun IoT shadow; receiveShadowCommand reads back whatever the device replied.
        // Hypothesis: poking the shadow forces a fresh telemetry sample / faster reporting.
        const { serial, command = "0", poke = true, receive = true } = req.body || {};
        if (!serial) return res.status(400).json({ error: "serial required" });
        const out = { serial, command };
        if (poke) {
          const setBody = { GoodsID: serial, Command: String(command) };
          setBody.sign = makeSign(setBody);
          try { out.set = await midnitePost("/Aliyuniotapi/iot/setShadowCommand", setBody, auth.token); }
          catch (e) { out.set = { error: e.message }; }
        }
        if (receive) {
          const recvBody = { GoodsID: serial };
          recvBody.sign = makeSign(recvBody);
